<script lang="ts">
	import { Breadcrumb, BreadcrumbItem, Heading, P, Select, Button, Toggle } from 'flowbite-svelte';
  ...
  export let data: PageData;
	...
	const contestTypes = Object.keys(formattedData).map(name => {return {value: name.split(" ").join(""), name}});
	...
	function randomize() {
		if (randomContestType)
			...
			selectedType = selectedType == '' ? "AMC8" : selectedType;
		selectedContest = formattedData[selectedType.split("C").join("C ")][randomBetween(0, formattedData[selectedType.split("C").join("C ")].length - 1)].value;
	}
...
<div class="flex mb-4">
  <Select items={contestTypes} bind:value={selectedType} placeholder="Contest type" class="!rounded-r-none !w-auto !pl-4"></Select>
  <Select items={formattedData[selectedType.split("C").join("C ")]} placeholder="Choose a contest" bind:value={selectedContest} class="!rounded-l-none !pl-4"/>
...
<Button on:click={goToContest} disabled={!selectedContest}>Go</Button>
